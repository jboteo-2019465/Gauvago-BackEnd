...
export const getHotelRequest = async (req, res) => {
  try {
    ...
    return res.send({ hotelRequest });
  } catch (err) {
    console.error(err);
    return res.status(500).send({ message: 'Error getting hotel request' });
  }
}
...
export const registerHotel = async (req, res) => {
  try {
    let data = req.body;
    let hotelRequest = await HotelRequest.findOne({ nameHotel: data.nameHotel });
    if (hotelRequest) {
      let hotel = new Hotel({
      ...
      return res.status(400).send({ message: 'The hotel request not found' });
    }
  } catch (err) {
    console.error(err);
